...
func TestMultiCheck(t *testing.T) {
	...
	checker := checker.NewMockChecker(ctrl)
	checker.EXPECT().Get(gomock.Eq("foo")).Return(nil)
	checker.EXPECT().Get(gomock.Eq("bar")).Return(nil)

	router := New(checker).newRouter()

	req, err := http.NewRequest("GET", "/v1/check/foo,bar", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMultiCheckFailed(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	checker := checker.NewMockChecker(ctrl)
	checker.EXPECT().Get(gomock.Eq("foo")).Return(nil)
	checker.EXPECT().Get(gomock.Eq("bar")).Return(fmt.Errorf("check failed"))
